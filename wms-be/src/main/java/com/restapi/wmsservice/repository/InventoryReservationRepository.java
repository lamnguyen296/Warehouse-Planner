...
public interface InventoryReservationRepository extends JpaRepository<InventoryReservation, Long> {
    ...
    @EntityGraph(attributePaths = {"planningDetail", "recycleOrder", "item", "warehouse", "inventory", "inventory.location"})
    ...
    @EntityGraph(attributePaths = {"recycleOrder", "item", "warehouse", "inventory", "inventory.location"})
    ...
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @EntityGraph(attributePaths = {"planningDetail", "recycleOrder", "item", "warehouse", "inventory", "inventory.location"})
    @Query("SELECT r FROM InventoryReservation r WHERE r.recycleOrder.id = :recycleOrderId " +
           "AND r.status = :status ORDER BY r.id")
    List<InventoryReservation> findByRecycleOrderAndStatusForUpdate(
            @Param("recycleOrderId") Long recycleOrderId,
