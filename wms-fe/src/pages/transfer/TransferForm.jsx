...
const TransferForm = ({ items, warehouses, plannings, onSubmit, onCancel }) => {
  ...
  const availableItems = useMemo(() => items.filter((item) => item.status === 'ACTIVE' && (!planned || item.itemType === 'SET')), [items, planned]);
  ...
  return <form className="form-stack" onSubmit={submit}>
    ...
    <label className="input-group"><span className="input-label">Item</span><select className="input-field" value={form.itemId} onChange={(event) => setField('itemId', event.target.value)} required><option value="">Select item</option>{availableItems.map((item) => <option key={item.id} value={item.id}>{item.code} - {item.name}</option>)}</select></label>
    <Input label="Transfer quantity" type="number" min="1" value={form.quantity} onChange={(event) => setField('quantity', event.target.value)} required />
