...
const TransferList = () => {
  ...
  const destinationLocations = useMemo(() => locations.filter((location) =>
    location.warehouseId === completing?.toWarehouse?.id && location.status !== 'MAINTENANCE'
  ...
  return <div><PageHeader title="Transfer & Issue" subtitle="Move stock between warehouses; SET warehouse to workshop is recorded as ISSUE" actions={canExecute && <Button onClick={() => setCreateOpen(true)}><Plus size={16} /> Create transfer</Button>} /><div className="surface"><Table columns={columns} data={data} /></div>
    <Modal isOpen={createOpen} onClose={() => setCreateOpen(false)} title="Create transfer"><TransferForm items={items} warehouses={warehouses} plannings={plannings} transfers={data} onSubmit={create} onCancel={() => setCreateOpen(false)} /></Modal>
