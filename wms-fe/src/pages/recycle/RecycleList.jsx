...
const RecycleList = () => {
  ...
  const load = useCallback(async () => { const [orders, itemData] = await Promise.all([recycleService.getAll(), itemService.getAll()]); setData(orders || []); setItems(itemData || []); }, []);
  useEffect(() => { load(); }, [load]);
  ...
  const columns = [
    ...
    { header: 'Conversion', accessor: 'conversion', render: (row) => `${row.fromItem?.code} -> ${row.toItem?.code}` },
  ...
  return <div><PageHeader title="Recycle Orders" subtitle="Convert raw components into finished components with actual-yield tracking" actions={canExecute && <Button onClick={() => setCreateOpen(true)}><Plus size={16} /> Manual recycle</Button>} /><div className="surface"><Table columns={columns} data={data} /></div>
    <Modal isOpen={createOpen} onClose={() => setCreateOpen(false)} title="Manual recycle order"><RecycleForm items={items} onSubmit={create} onCancel={() => setCreateOpen(false)} /></Modal>
