...
const RecycleForm = ({ items, boms, onSubmit, onCancel }) => {
  ...
  const compatibleOutputIds = useMemo(() => new Set(
    boms.filter((bom) => bom.parentItemId === Number(form.fromItemId))
      .map((bom) => bom.childItemId)
  ), [boms, form.fromItemId]);
  const finishedItems = items.filter((item) => item.itemType === 'FINISHED_COMPONENT'
    && item.status === 'ACTIVE' && compatibleOutputIds.has(item.id));
  ...
  return <form className="form-stack" onSubmit={submit}>
    ...
    <label className="input-group"><span className="input-label">Finished component output</span><select className="input-field" value={form.toItemId} onChange={(event) => setField('toItemId', event.target.value)} required disabled={!form.fromItemId}><option value="">{form.fromItemId ? 'Select compatible output' : 'Select raw input first'}</option>{finishedItems.map((item) => <option key={item.id} value={item.id}>{item.code} - {item.name}</option>)}</select></label>
